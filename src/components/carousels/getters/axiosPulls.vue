  ...
  <div class="listElement" v-bind:id="item.carousel">
   ...
   <div class="carouselContainer">
    ...
    <ul class="carouselListElement">
     <li v-for="object of item.ApiData" :key="object" class="card carouselList" :id="object.id">
      ...
      <div :id="'modal' + object.id" class="movieModal noshow">
       ...
       <p>{{ object.overview }}</p>
       <button @click="addToMyList(object)" class="myListBtn">Add to My List</button>
      </div>
     </li>
    </ul>
...
export default {
 ...
 methods: {
  async pullAll() {
   await axios.all(this.requestLinks.map((link) => axios.get(link))).then((data) => {
    this.AllPulls = data;
    if (this.AllPulls[1].data.results.title === undefined) {
     this.AllPulls[1].data.results.title = this.AllPulls[1].data.results.name;
    }
